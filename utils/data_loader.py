...
from data.all_position_stats import (get_bulk_position_stats,
                                     get_simple_position_stats)
...
def load_trades():
    ...
    SELECT transaction_date AS date, team1,acquired1 AS acquired_team1,team2,
    acquired2 AS acquired_team2 FROM MergedRows
...
def load_players():
    ...
    SELECT player_id, name, headshot_url, season, birthdate,
    college, draft_year,
...
# this returns the entire range of seasons of a player
...
def get_win_loss():
    ...
    SELECT
        season, week, gameday, recent_team AS team,
        player_display_name AS name, player_id,
...
def get_league_per_game_average(position, start_date, end_date):
    ...
    """
    data = {'_position': position, '_start_date': start_date,
            '_end_date': end_date}
    df = pd.read_sql_query(query, engine, params=data)
    return df


# this gets the ranked performance of a player during their tenure at all
...
def get_ranked_players_per_team(position):
    ...
    SELECT
        ...
        q.percent_rank
    FROM (
        SELECT DISTINCT player_id, player_display_name AS name,
        recent_team AS team
        FROM nfl_data
    ) p
